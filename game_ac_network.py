    ...
    def prepare_loss(self, entropy_beta):
        with tf.device(self._device):
            ...
            policy_loss = - tf.reduce_sum(
                tf.reduce_sum(tf.multiply(log_pi, self.a), reduction_indices=1)
                * self.td + entropy * entropy_beta
            )
...
class GameACLSTMNetwork(GameACNetwork):
    ...
    def run_policy(self, sess, s_t):
        # This run_policy() is used for
        # displaying the result with display tool.
    ...
    def run_value(self, sess, s_t):
        ...
        # When next sequcen starts, V will be calculated again with the same
